...
//        session()->flash('verify', 'Your Request submitted successfully.');
    ...
    public function candidateDashboard()
    {
        ...
        $applied_jobs = Applied_Jobs::where('candidate_id', $Candidate->id)->count();
...
//            return response()->json(['url'=>url('/account/logout')]);
    ...
    public function findUserJobs()
    {
        ...
        $candidate_resumes = candidate_resume::where('candidate_id', session()->get('candidate_id'))->get();
        $industries = Industry::all();
        $jobs = user_job::with('Client')->where('featured', 1)->whereDate('applied_before', '>', Carbon::now())->where('job_approved', 1)->get();
//        dd('find user job');
    ...
    public function searchedJobs(Request $request)
    {
        ...
        if (is_null($request->job_title) && is_null($request->place)) {
            if ($request->sel_option == 2) {
                ...
                $find_jobs = user_job::with('Client')->whereRaw($where_filter)->whereDate('applied_before', '>', Carbon::now())->where('job_approved', 1)->orderBy($order_by_col, $order_by_action)
                    ->offset($start)->limit($request->length)->get();
            }
            $total_jobs = user_job::whereRaw($where_filter)->whereDate('applied_before', '>', Carbon::now())->where('job_approved', 1)->get();
        ...
        if (!is_null($request->job_title) || !is_null($request->place)) {
            if ($request->sel_option == 2) {
                ...
                $find_jobs = user_job::with('Client', 'industry')->orderBy($order_by_col, $order_by_action)
                    ->where($where)->whereRaw($where_filter)->whereDate('applied_before', '>', Carbon::now())->where('job_approved', 1)->offset($start)->limit($request->length)->get();
            }
            $total_jobs = user_job::where($where)->whereDate('applied_before', '>', Carbon::now())->where('job_approved', 1)->whereRaw($where_filter)->get();
        }
    ...
    {
        $recruitment = recruitment_service::with('industry')->where('client_id', session('c_email.id'))->get();
//        return view('user.company_dashboard.recruitment')->with(compact('recruitment'));
        return view('user.company_dashboard.new-active-recruitments')->with(compact('recruitment'));
    }
...
//        return view('user.company_dashboard.recruitment_details')->with(compact('recruitment'));
    ...
    {
        $job = user_job::with('Client', 'industry', 'education')->where('id', $request->id)->first();
        $candidate = '';;
        $candidate_resumes = '';
        $job_check = '';
        $candidate_fav_job = '';
    ...
    public function jobDetails($id)
    {
        ...
        return view('user.job-details')->with(compact('job', 'candidate', 'candidate_resumes', 'job_check'));
    }
...
//                'location' => 'required|regex:/^[a-zA-Z,.\s]*$/|min:2|max:255',
            ...
            $job_check = Applied_Jobs::where('job_id', $request->id)->where('candidate_id', session()->get('candidate_id'))->first();
            $job_chk_last_date = user_job::where('id', $request->id)->first();
...
//            'web_url' => 'required|url',
    ...
    {
//        return view('user.company_dashboard.active_jobs');
    ...
    public function activeJobs(Request $request)
    {
        ...
        //$jobs = user_job::with('client')->where('client_id' , session('c_email.id'))->whereDate('applied_before','>',Carbon::now())->orderBy('posted_at','desc')->simplePaginate(5) ;
        if ($request->current == 1) {
            $start = 0;
        } else {
            $start = ($request->current - 1) * $request->length;
        ...
        $jobs = user_job::with('client')->where('client_id', session('c_email.id'))->where('job_approved', 1)->whereDate('applied_before', '>', Carbon::now())->orderBy('id', 'desc')->offset($start)->limit($request->length)->get()->toArray();
        $arr = [];
        foreach ($jobs as $job) {
        ...
        $arrayName = array('0' => '', '1' => $jobs, '2' => $arr);
        return json_encode($arrayName);
    ...
    public function jobViewDetails($id)
    {
        ...
        $candidates_applied = Applied_Jobs::with('candidate', 'candidate.education')->where('job_id', $id)->get();
...
//        return view('user.company_dashboard.expired_jobs');
    ...
    public function expiredJobs(Request $request)
    {
        ...
        $jobs = user_job::with('client', 'industry')->where('client_id', session('c_email.id'))->where('job_approved', 1)->orderBy('applied_before', 'desc')->whereDate('applied_before', '<=', Carbon::now())->offset($start)->limit($request->length)->get()->toArray();
        $arrayName = array('0' => '', '1' => $jobs);
        return json_encode($arrayName);
    }
...
//                'location' => 'required|regex:/^[a-zA-Z,.\s]*$/|min:2|max:255',
    ...
    public function nonLoggedinJobApply(Request $request)
    {
        ...
        $job_details = user_job::with('client')->where('id', $request->id)->first();
