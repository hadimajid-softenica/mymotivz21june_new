                    ...
                    <div class="col-md-4">
                        <div class="motivz-typo-wrap candidate">
                            <figure class="motivz-jobdetail-list">
                                <span class="motivz-jobdetail-listthumb"><small style="background-position: top; background-image: url('@if(!empty($candidate->prof_image)){{asset('/uploads/Candidate_Profile_Images/'.$candidate->prof_image)}} @else {{asset('user/images/avatar1.png')}} @endif');"></small></span>
                                <figcaption>
                                    <h2>{{$cand_apply_job->name}} <small>( @if(!empty($candidate->job_title)){{$candidate->job_title}}@else N/A @endif )</small></h2>
                                    <ul class="motivz-jobdetail-options">
                                        <li><i class="fa fa-map-marker"></i>{{$cand_apply_job->location}}</li>
                                        <li><i class="fa fa-envelope"></i> {{$candidate->email}}</li>
                                        <li><i class="fa fa-phone"></i> {{$cand_apply_job->phone}}</li>
                                        @if($candidate->linkedin_url)
                                            <li><a href="{{$candidate->linkedin_url}}" style="color: #4d9a10 !important;" target="_blank" class="fa fa-linkedin"></a></li>
                                        @endif
                                    </ul>
                                </figcaption>
                            </figure>
                                ...
                                <ul>
                                    <li>
                                        ...
                                        @if(!empty($candidate->job_title))
                                            ...
                                            @endphp
                                            <p>@foreach($job_title as $job_title)<span>{{$job_title}}</span>@endforeach</p>
                                        @else
                                            <p><span>N/A</span></p>
                                        @endif
                                    </li>
...
{{--                                        @endif--}}
                                    ...
                                    <li>
                                        ...
                                        @if(!empty($candidate->interest))
                                            ...
                                            @endphp
                                            <p>@foreach($interests as $interest)<span>{{$interest}}</span>@endforeach</p>
                                        @else
                                            <p><span>N/A</span></p>
                                        @endif
                                    </li>
                                    <li>
                                        ...
                                        @if(!empty($candidate->certifications))
                                            ...
                                            @endphp
                                            <p>@foreach($certifications as $certification)<span>{{$certification}}</span>@endforeach</p>
                                        @else
                                            <p><span>N/A</span></p>
                                        @endif
                                    </li>
                                    <li>
                                        ...
                                        <p>@if(!empty($candidate->experience)){{$candidate->experience}}@else N/A @endif</p>
                                    </li>
                                    <li>
                                        <span>Education</span>
                                        @if(!empty($candidate->education_id))<p>{{$candidate->education->name}}</p> @else <p>N/A</p> @endif
                                    </li>
                                    <li>
                                        <span>Industry</span>
                                        <p>@if(!empty($candidate->industry_id)){{$candidate->industry->name}}@else N/A @endif</p>
                                    </li>
                                    <li>
                                        <span>Authorization Status</span>
                                        <p>@if(!empty($candidate->auth_status)){{$candidate->auth_status}}@else N/A @endif</p>
                                    </li>

                                </ul>
                            </div>
                            <a href="/files/{{$cand_resume->resume}}" id="print" class="simple-btn" download>Download Resume</a>
                        </div>
                    </div>
                </div>
            </div>
